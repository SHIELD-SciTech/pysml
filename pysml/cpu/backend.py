...
def uniform(low=0.0, high=1.0, size=None, dtype=np.float32, device=None):

    return np.random.uniform(low, high, size).astype(dtype)

def normal(mean=0.0, std=1.0, size=None, dtype=np.float32, device=None):

    return np.random.normal(mean, std, size).astype(dtype)
...
def batch_norm(x, running_mean=None, running_var=None, weight=None, bias=None,
    ...
    if training:
        ...
        mean = running_mean.reshape(shape_4d) if running_mean is not None else np.mean(x, axis=axes, keepdims=True)
        var = running_var.reshape(shape_4d) if running_var is not None else np.var(x, axis=axes, keepdims=True)
...
def dropout(x, p=0.5, training=True):
    ...
    mask = rand(x.shape, dtype=x.dtype) > p
...
def linspace(start, stop, num, dtype=np.float32):
