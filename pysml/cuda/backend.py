...
def randint(low, high=None, size=None, dtype=None, device=None):
    ...
    if dtype is None:
        dtype = cp.int64
    return cp.random.randint(low, high, size=size, dtype=dtype)

def uniform(low=0.0, high=1.0, size=None, dtype=None, device=None):

    if dtype is None:
        dtype = cp.float32
    return cp.random.uniform(low, high, size).astype(dtype)

def normal(mean=0.0, std=1.0, size=None, dtype=None, device=None):

    if dtype is None:
        dtype = cp.float32
    return cp.random.normal(mean, std, size).astype(dtype)
...
def batch_norm(x, running_mean=None, running_var=None, weight=None, bias=None,
    ...
    if training:
        ...
        mean = running_mean.reshape(shape_4d) if running_mean is not None else cp.mean(x, axis=axes, keepdims=True)
        var = running_var.reshape(shape_4d) if running_var is not None else cp.var(x, axis=axes, keepdims=True)
...
def embedding_lookup(table, indices, padding_idx=None):
    ...
    if padding_idx is not None:
        ...
        result = result.copy()  # Need copy for in-place modification
        result[mask] = 0
...
def linspace(start, stop, num, dtype=None):
    ...
    if dtype is None:
        dtype = cp.float32
    return cp.linspace(start, stop, num, dtype=dtype)
