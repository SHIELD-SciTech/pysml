...
def tanh(input, out=None):
        ...
        # Don't save output - backward will recompute to save memory
        _attach_grad_fn(out, backward_tanh, [input], metadata={})
        return out
...
def sigmoid(input, out=None):
        ...
        # Don't save output - backward will recompute to save memory
        _attach_grad_fn(out, backward_sigmoid, [input], metadata={})
        return out
...
def sin(input, out=None):
        ...
        _attach_grad_fn(out, backward_sin, [input])
        return out
...
def cos(input, out=None):
        ...
        _attach_grad_fn(out, backward_cos, [input])
        return out
...
# ============================================================================
# Differentiable reduction wrappers
# ============================================================================
...
def abs(input, out=None):
        ...
        _attach_grad_fn(out, backward_abs, [input])
        return out
...
def floor_divide(input, other, out=None):
        ...
        if out is None:
                ...
                out = _wrap_result(input, backend, False, result_data)  # Not differentiable
        else:
...
def remainder(input, other, out=None):
        ...
        if out is None:
                ...
                out = _wrap_result(input, backend, False, result_data)
        else:
...
def sign(input, out=None):
        ...
        _attach_grad_fn(out, backward_sign, [input])
        return out
...
def concatenate(tensors, axis=0):
        ...
        out = _wrap_result(tensors[0], backend, any(t._requires_grad for t in tensors), result_data)
...
def where(condition, x, y):
        ...
        template = x if hasattr(x, '_dtype') else y
        out = _wrap_result(template, backend, _requires_grad(x) or _requires_grad(y), result_data)
...
def batch_norm(input, running_mean=None, running_var=None, weight=None, bias=None,
        ...
        if training:
                mean_val = backend.mean(input.data, axis=axes, keepdims=True)
                centered = backend.subtract(input.data, mean_val)
                ...
                mean_val = backend.reshape(running_mean, shape_4d) if running_mean is not None else backend.mean(input.data, axis=axes, keepdims=True)
                var = backend.reshape(running_var, shape_4d) if running_var is not None else backend.mean(
                        backend.multiply(
                                backend.subtract(input.data, mean_val),
                                backend.subtract(input.data, mean_val),
                        ),
                        axis=axes,
                        keepdims=True,
        ...
        if is_grad_enabled() and out._requires_grad and training:
                out._grad_fn = Function(
                        ...
                        metadata={'eps': eps, 'gamma': weight_data, 'mean': mean_val, 'var': var, 'axes': axes}
                )
...
def dropout(input, p=0.5, training=True, out=None):
        ...
        if out is None:
                if training and p > 0:
                        keep_prob = 1.0 - p
                        rand_vals = backend.rand(input.shape, dtype=input.data.dtype, device=input.device)
                        mask = backend.greater(rand_vals, p)
...
def embedding(weight, indices, padding_idx=None):
        ...
        out = _wrap_result(weight, backend, weight._requires_grad, result_data)
...
def split(input, split_size_or_sections, dim=0):
        ...
        for idx, chunk_data in enumerate(chunks_data):
                chunk = _wrap_result(input, backend, input._requires_grad, chunk_data)
                chunks.append(chunk)
