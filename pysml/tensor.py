...
class Tensor:
    ...
    def backward(self, gradient: Optional["Tensor"] = None, retain_graph: bool = False) -> None:
        ...
        with self._grad_lock:
            if self._grad is None:
                ...
                if hasattr(backend, "add_"):
                    ...
                    backend.add(grad_buffer, increment, out=grad_buffer)
        ...
        # Release computational graph to free memory (unless retain_graph=True)
        if not retain_graph:
            for node in topo_order:
                if node._grad_fn is not None:
                    node._grad_fn.release_graph()
                    node._grad_fn = None
                # Clear intermediate gradients for non-leaf tensors
                if not node._requires_grad or node._grad_fn is not None:
                    node._grad = None
    ...
    def expand(self, *sizes) -> "Tensor":
        ...
        if len(sizes) == 1 and isinstance(sizes[0], (tuple, list)):
            sizes = tuple(sizes[0])
        result_data = self._backend.broadcast_to(self.data, sizes)
        return self._new_like(result_data, requires_grad=self._requires_grad)

    def repeat(self, *sizes) -> "Tensor":

        if len(sizes) == 1 and isinstance(sizes[0], (tuple, list)):
            sizes = tuple(sizes[0])
        result_data = self._backend.tile(self.data, sizes)
        return self._new_like(result_data, requires_grad=self._requires_grad)
    ...
    def std(self, axis=None, keepdims: bool = False):
        ...
        result_data = self._backend.std(self.data, axis=axis, keepdims=keepdims)
        return self._new_like(result_data, requires_grad=False)

    def var(self, axis=None, keepdims: bool = False):

        result_data = self._backend.var(self.data, axis=axis, keepdims=keepdims)
        return self._new_like(result_data, requires_grad=False)

    def argmax(self, axis=None):

        result = self._backend.argmax(self.data, axis=axis)
        return result

    def argmin(self, axis=None):

        result = self._backend.argmin(self.data, axis=axis)
        return result
    ...
    def __eq__(self, other):
        ...
        return self._backend.equal(self.data, other_data)

    def __ne__(self, other):

        other_data = other.data if isinstance(other, Tensor) else other
        return self._backend.not_equal(self.data, other_data)

    def __lt__(self, other):

        other_data = other.data if isinstance(other, Tensor) else other
        return self._backend.less(self.data, other_data)

    def __le__(self, other):

        other_data = other.data if isinstance(other, Tensor) else other
        return self._backend.less_equal(self.data, other_data)

    def __gt__(self, other):

        other_data = other.data if isinstance(other, Tensor) else other
        return self._backend.greater(self.data, other_data)

    def __ge__(self, other):

        other_data = other.data if isinstance(other, Tensor) else other
        return self._backend.greater_equal(self.data, other_data)
    ...
    def add_(self, other, alpha=1):
        ...
        other_data = other.data if isinstance(other, Tensor) else other
        if alpha != 1:
            other_data = self._backend.multiply(other_data, alpha)
        self._backend.add(self.data, other_data, out=self.data)
        self._bump_version()
        return self
    ...
    def sub_(self, other):
        ...
        other_data = other.data if isinstance(other, Tensor) else other
        self._backend.subtract(self.data, other_data, out=self.data)
        self._bump_version()
        return self
    ...
    def mul_(self, other):
        ...
        other_data = other.data if isinstance(other, Tensor) else other
        self._backend.multiply(self.data, other_data, out=self.data)
        self._bump_version()
        return self
    ...
    def div_(self, other):
        ...
        other_data = other.data if isinstance(other, Tensor) else other
        self._backend.divide(self.data, other_data, out=self.data)
        self._bump_version()
        return self
    ...
    def copy_(self, src):
        ...
        src_data = src.data if isinstance(src, Tensor) else src
        self._backend.copyto(self.data, src_data)
        self._bump_version()
        return self
