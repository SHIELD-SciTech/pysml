...
def backward_add(grad_output, input_ref, other_ref, **metadata):
	...
	if other_tensor and other_tensor._requires_grad:
		# Handle broadcasting: sum over extra dimensions if needed
		grad = grad_output
		
		# If shapes differ due to broadcasting, reduce gradient
		if hasattr(grad, 'shape') and hasattr(other_tensor, 'shape'):
			if grad.shape != other_tensor.shape:
				# Sum over broadcast dimensions
				backend = grad._backend if hasattr(grad, '_backend') else other_tensor._backend
				grad_data = grad.data if hasattr(grad, 'data') else grad
				
				# Reduce dimensions that were broadcast
				ndim_diff = len(grad.shape) - len(other_tensor.shape)
				if ndim_diff > 0:
				...
				for i in range(len(other_tensor.shape)):
					if other_tensor.shape[i] == 1 and grad_data.shape[i] != 1:
						grad_data = backend.sum(grad_data, axis=i, keepdims=True)
				
				# Create new tensor with reduced gradient
				if hasattr(grad, '_backend'):
					grad_reduced = type(grad).__new__(type(grad))
					grad_reduced._backend = backend
					grad_reduced._dtype = grad._dtype if hasattr(grad, '_dtype') else None
					grad_reduced.device = grad.device if hasattr(grad, 'device') else None
					grad_reduced.active_device = grad.active_device if hasattr(grad, 'active_device') else 'cpu'
...
def backward_multiply(grad_output, input_ref, other_ref, **metadata):
	...
	if input_tensor and input_tensor._requires_grad:
		...
		if other_tensor:
			...
			grad.device = grad_output.device
			grad.active_device = grad_output.active_device
			grad.data = backend.multiply(grad_output.data, other_tensor.data)
			grad._requires_grad = False
			grad._grad = None
			grad._grad_fn = None
			...
			grad.device = grad_output.device
			grad.active_device = grad_output.active_device
			grad.data = backend.multiply(grad_output.data, scalar)
			grad._requires_grad = False
			grad._grad = None
			grad._grad_fn = None
			grads.append((input_tensor, grad))
	...
	if other_tensor and other_tensor._requires_grad:
		...
		grad.device = grad_output.device
		grad.active_device = grad_output.active_device
		grad.data = backend.multiply(grad_output.data, input_tensor.data)
		grad._requires_grad = False
		grad._grad = None
		grad._grad_fn = None
		grads.append((other_tensor, grad))
...
def backward_divide(grad_output, input_ref, other_ref, **metadata):
	...
	if input_tensor and input_tensor._requires_grad:
		...
		if other_tensor:
			...
			grad.device = grad_output.device
			grad.active_device = grad_output.active_device
			grad.data = backend.divide(grad_output.data, other_tensor.data)
			grad._requires_grad = False
			grad._grad = None
			grad._grad_fn = None
			grads.append((input_tensor, grad))
			...
			grad.device = grad_output.device
			grad.active_device = grad_output.active_device
			grad.data = backend.divide(grad_output.data, scalar)
			grad._requires_grad = False
			grad._grad = None
			grad._grad_fn = None
			grads.append((input_tensor, grad))
	...
	if other_tensor and other_tensor._requires_grad:
		...
		# -grad_output * input
		temp = backend.multiply(grad_output.data, input_tensor.data)
		temp = backend.negative(temp)
		
		# other^2
		other_squared = backend.multiply(other_tensor.data, other_tensor.data)
		
		# final gradient
		grad.data = backend.divide(temp, other_squared)
		grad._requires_grad = False
		grad._grad = None
		grad._grad_fn = None
		grads.append((other_tensor, grad))
...
def backward_sum(grad_output, input_ref, **metadata):
	...
	if input_tensor and input_tensor._requires_grad:
		...
		# For now, simple implementation: fill with grad_output value
